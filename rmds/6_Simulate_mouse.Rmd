...
p_shannon <- data_shannon %>% 
  ggplot(aes(x = data, 
             y = Shannon)) +
  geom_boxplot(aes(color = data), outlier.shape = NA) +
...
cowplot::plot_grid(p, p_shannon, p_ratio, p_abs,
                   labels = c("A", "B", "C", "D"),
                   rel_widths = c(2, 1, 1, 1),
                   nrow = 1) %>% 
ggsave("figures/Fig5/Fig5.pdf", ., width = 12, height = 5)
```